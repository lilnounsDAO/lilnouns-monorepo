...
async function deploy() {
  [deployer, bidderA, wethDeployer, noundersDAO, lilNoundersDAO] = await ethers.getSigners();
...
describe('End to End test with deployment, auction, proposing, voting, executing', async () => {
  ...
  it('sets all starting params correctly', async () => {
    ...
    expect(await nounsToken.nounsDAO()).to.equal(noundersDAO.address);
    expect(await nounsToken.lilnoundersDAO()).to.equal(lilNoundersDAO.address);
    ...
    // minted to lilnounders and nounders
    ...
    expect(await nounsToken.ownerOf(1)).to.equal(noundersDAO.address);
